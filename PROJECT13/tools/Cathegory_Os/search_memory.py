...
ImportanceFilter = Union[int, Dict[str, int]]
EmotionFilter = Dict[str, Dict[str, float]]
...
async def search_memory(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        # Convert string-based filters to appropriate formats
        args['importance_filter'] = _parse_importance_filter(args.get('importance_filter'))
        args['emotion_filter'] = _parse_emotion_filter(args.get('emotion_filter'))

        results = await _search_memory(**args)
...
def _parse_importance_filter(importance_filter: Union[str, Dict[str, int]]) -> Optional[ImportanceFilter]:
    """Convert string-based importance filter to the appropriate format."""
    if isinstance(importance_filter, str):
        try:
            return int(importance_filter)
        except ValueError:
            return {'min': importance_filter}  # Assuming string represents minimum importance
    return importance_filter

def _parse_emotion_filter(emotion_filter: Union[str, Dict[str, Any]]) -> Optional[EmotionFilter]:
    """Convert string-based emotion filter to the appropriate format."""
    if isinstance(emotion_filter, str):
        return {emotion_filter: {'minimum': 0.5}}  # Assuming string represents emotion with minimum threshold
...
class MemoryFrameSearcher:
    ...
    def _apply_filters(
        ...
        for filter_name, filter_value in filters.items():
            if filter_value is None:
                continue  # Skip filters with None value
    ...
    def _calculate_relevance_score(self, memory_frame: Dict[str, Any], query: str, query_type: str, query_fields: Optional[List[str]], query_operator: str) -> float:
        if query_fields is None:
            content = json.dumps(memory_frame)
        else:
            content = json.dumps({field: memory_frame.get(field, '') for field in query_fields})
        ...
        if query_type == "keyword":
            ...
            # Implement semantic search here. For now, we'll use a simple approximation.
    ...
    async def search_memory_frames(
        ...
        for filename in os.listdir(self.memories_folder_path):
            if filename.endswith('.json'):
                ...
                tasks.append(self._process_memory_frame(file_path, file_info, query, query_type, query_fields, query_operator, filters))
...
search_memory_description_json = {
  "name": "search_memory",
  "description": "Searches the memory store for relevant information using advanced querying options.",
  "parameters": {
    "type_": "OBJECT",
    "properties": {
      "query": {
      ...
      "query_type": {
        "type_": "STRING",
        "description": "The type of query to perform. Options are: 'keyword', 'semantic', 'regex'."
      },
      "query_fields": {
        "type_": "ARRAY",
        "items": {
      ...
      "query_operator": {
        "type_": "STRING",
        "description": "The logical operator to use when combining multiple query terms. Options are: 'AND', 'OR'."
      },
      "max_results": {
        "type_": "INTEGER",
...
search_memory_description_short_str = "Searches memory frames within a specified folder based on provided criteria, using advanced querying options including keyword, semantic, and regex search types."
