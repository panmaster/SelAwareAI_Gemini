...
def update_prompts(prompt_key: str, new_prompt: str) -> dict:
    """Updates a prompt in the stage_prompts.json file."""

    print(f"{BLUE}Entering: UpdatePrompts(...) {RESET}")
    try:
        # Load existing stage_prompts
        with open("Brain_settings/stage_prompts.json", 'r') as file:
            prompts = json.load(file)

        # Update the specified prompt
        prompts[prompt_key] = new_prompt

        # Save updated stage_prompts
        with open("Brain_settings/stage_prompts.json", 'w') as file:
        ...
        error_message = f"File 'stage_prompts.json' not found."
        print(f"{RED}{error_message} {RESET}")
        print(f"{BLUE}Exiting: UpdatePrompts(...) {RESET}")
        return {"status": "failure", "message": error_message}

    except KeyError:
        error_message = f"Prompt '{prompt_key}' not found in 'stage_prompts.json'."
        print(f"{RED}{error_message} {RESET}")
        print(f"{BLUE}Exiting: UpdatePrompts(...) {RESET}")
        return {"status": "failure", "message": error_message}
...
update_prompts_description_json = {
  "function_declarations": [
    {
      "name": "update_prompts",
      "description": "Updates a prompt in the 'stage_prompts.json' file.",
      "parameters": {
        "type_": "OBJECT",
        "properties": {
          "prompt_key": {
...
update_prompts_description_short_str = "Updates a prompt in the 'stage_prompts.json' fil"