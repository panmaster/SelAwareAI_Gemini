...
def create_embedded_modelium_function(
    ...
    # --- Main Function: run_modelium ---
    template_run_modelium = f"""
    def run_modelium(
        user_input: str,
        ...
        while True:
            ...
            {  # Start of Dynamic Model Section
                # Model Initialization 
                ''.join([
                    f"""
                    # --- {model_config['model_name']} Stage ---
                    print_colored(Color.OKBLUE, f"--- {model_config['model_name']} Stage ---")
                    """  +
                    (
                        f"""
                        prompt_{model_config['model_name']} = f"previous responses: {{previous_responses}}" +  \
                                        f"{model_config['prompt']}"
                        prompt_{model_config['model_name']} = prompt_{model_config['model_name']}.replace("{{user_input}}", user_input)
                        """
                        if i == 0 else
                        f"""
                        prompt_{model_config['model_name']} = f"{model_config['prompt']}"
                        prompt_{model_config['model_name']} = prompt_{model_config['model_name']}.replace("{{previous_responses}}", " ".join(previous_responses))
                        prompt_{model_config['model_name']} = prompt_{model_config['model_name']}.replace("{{{model_configs[i-1]['model_name']}_text}}", {model_configs[i-1]['model_name']}_text)
                        """
                    ) +
                    f"""
                    {model_config['model_name']}_response = {model_config['model_name']}_chat.send_message(prompt_{model_config['model_name']}, tools=[tool_manager.retrieve_tools_by_names] if "{model_config['model_name']}" == "Planner" else [])
                    {model_config['model_name']}_text = extract_text_from_response({model_config['model_name']}_response)
                    print({model_config['model_name']}_response)
                    print_colored(Color.OKGREEN, f"{model_config['model_name']}'s Response: {{{model_config['model_name']}_text}}")
                    all_responses.setdefault('{model_config['model_name']}', []).append({model_config['model_name']}_text)

                    {model_config['model_name']}_results = INTERPRET_function_calls({model_config['model_name']}_response, tool_manager)
                    print_colored(Color.OKCYAN, f"{model_config['model_name']}'s Function Calls: {{{model_config['model_name']}_results}}")
                    all_results.setdefault('{model_config['model_name']}', []).extend({model_config['model_name']}_results)
                    last_model_output = {model_config['model_name']}_text
                    current_loop_responses.append({model_config['model_name']}_text)
                    previous_responses.append(f"{model_config['model_name']}: {{{model_config['model_name']}_text}}")
                    current_loop_results.extend({model_config['model_name']}_results)

                    stop_flag, reason, flag = check_stop_flags({model_config['model_name']}_text)
                    if stop_flag:
    ...
    final_code = (
        ...
        template_check_flags +
        template_prepare_return +
        template_run_modelium
    )


    # --- Add Model Initialization Outside the Loop ---
