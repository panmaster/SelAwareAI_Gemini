...
function createModeliumStructure(modelium) {
    ...
    // Clear existing child nodes and edges
    ...
    for (let p = 0; p < modelium.parallelCount; p++) {
        ...
        for (let i = 0; i < modelium.chainLength; i++) {
            ...
            // Connect Modelium to the first Model only
            if (i === 0) {
                edges.add({
                    from: modelium.id,
                    to: modelNode.id,
            ...
            // Connect Result -> Interpreter -> Interpreter Result
            edges.add({
                from: resultNode.id,
            ...
            // Connect to next Model if not the last in chain
            if (i < modelium.chainLength - 1) {
                edges.add({
                    from: resultNode.id,
                    to: lastNodeId + 1,  // Next Model ID
                    smooth: { type: 'cubicBezier' }
                });
                edges.add({
                ...
                // Connect to the first Model in the current parallel chain for looping
                const firstModelId = modelNode.id - (modelium.chainLength - 1) * 4;

                edges.add({
                    from: resultNode.id,
                    to: firstModelId,
                    smooth: { type: 'cubicBezier', roundness: 1 },
                    dashes: [5, 5],
                    color: { color: '#ff0000' }
                });
        ...
        // Connect last Result and Interpreter Result to Return
        edges.add({
            from: lastResultId,
            to: returnNode.id,
            smooth: { type: 'cubicBezier' }
