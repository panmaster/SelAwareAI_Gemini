...
from tools.Cathegory_Os.RETRIVE_RELEVANT_FRAMES import RETRIEVE_RELEVANT_FRAMES

# Configuration

SESSION_FOLDER, MEMORY_FOLDER = "sessions", "memories"
MEMORY_STRUCTURE_SUMMARY_FILE = "memory_structure_summary.txt"
PROMPTS_FILE = os.path.join("../Gemini_SELF_AWARE/PROJECT_7/Brain_settings", "prompts.json")
EMOTIONS_FILE = os.path.join("../Gemini_SELF_AWARE/PROJECT_7/Brain_settings", "emotions.json")
...
class Evaluator:
    def __init__(self):
        ...
        self.model = AutoModelForSeq2SeqLM.from_pretrained("t5-base-finetuned-for-prompt-engineering")
        self.tokenizer = AutoTokenizer.from_pretrained("t5-base-finetuned-for-prompt-engineering")

        # Initialize storage for past data
        self.conversation_history = []
        self.prompts = self.load_prompts()
        self.tool_scores = {}  # Store tool performance scores
    ...
    def load_prompts(self):
        try:
            ...
            print("Error: prompts.json not found. Using default prompts.")
            return {
...
class GeminiSelfAwareAI:
    ...
    def gather_introspection_data(self):
        ...
        relevant_memories = RETRIEVE_RELEVANT_FRAMES(self.state_of_mind['FocusOn']) # No await needed
        ...
        while True:
            try:
                ...
                self.conversation_history.append({
                    ...
                    "tool_results": self.function_call_results,
                    "state_before": self.state_of_mind.copy(),
                    "state_after": self.state_of_mind.copy(),  # After state update
                    "emotions_before": self.emotions.copy(),
                    "emotions_after": self.emotions.copy()
                })

                self.evaluator.evaluate_loop(self.conversation_history, self.state_of_mind, self.emotions)

                # 3. Update prompts.json
                self.prompts = self.evaluator.get_updated_prompts()
                self.save_json(PROMPTS_FILE, self.prompts)
