"""1. initialisation:
2 checking for  existance of  memoreis folder:
  if  dolder  thos  not  exist  procede  to Full initialisaiton
  : creating  standard  folder  structure from template:
  : creating  maping
  : creating  current_folder_structure.txt
3. if folder  memoreis   do exists: then  checking  folder  structure    against:
    current_folder_structure.txt
    if   memoreis   structure  differs  from   current_folder_structure.txt
    then  new  maping  needs  to  intialsiation:
4: after  that  system  seams  to be  synchrosnied,
    both:
      current_folder_structure.txt  nad  maping  are  synchronised:"""
